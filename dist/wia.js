...
if (typeof Paho === 'undefined') {
  var _Paho = {}; // eslint-disable-line no-unused-vars
}
...
Paho.MQTT = function (global) {
    ...
    for (var key in obj) {
      if (obj.hasOwnProperty(key)) {
        if (keys.hasOwnProperty(key)) {
          if (_typeof(obj[key]) !== keys[key]) {
            throw new Error(format(ERROR.INVALID_TYPE, [_typeof(obj[key]), key]));
          }
        } else {
  ...
  */
    ...
    switch (this.type) {
      ...
      case MESSAGE_TYPE.CONNECT:
        switch (this.mqttVersion) {
          case 3:
            remLength = remLength + (MqttProtoIdentifierv3.length + 3);
            break;

          case 4:
            remLength = remLength + (MqttProtoIdentifierv4.length + 3);
            break;
        }

        remLength = remLength + (UTF8Length(this.clientId) + 2);

        if (this.willMessage != undefined) {
          remLength = remLength + (UTF8Length(this.willMessage.destinationName) + 2); // Will message is always a string, sent as UTF-8 characters with a preceding length.
          ...
          remLength = remLength + (willMessagePayloadBytes.byteLength + 2);
        }

        if (this.userName != undefined) {
          remLength = remLength + (UTF8Length(this.userName) + 2);
        }

        if (this.password != undefined) {
          remLength = remLength + (UTF8Length(this.password) + 2);
        }
      ...
      case MESSAGE_TYPE.SUBSCRIBE:
        first = first | 0x02; // Qos = 1;

        for (var i = 0; i < this.topics.length; i++) {
          topicStrLength[i] = UTF8Length(this.topics[i]);
          remLength = remLength + (topicStrLength[i] + 2);
        }

        remLength = remLength + this.requestedQos.length; // 1 byte for each topic's Qos
      ...
      case MESSAGE_TYPE.PUBLISH:
        ...
        first = first = first | this.payloadMessage.qos << 1;
        ...
        remLength = remLength + (destinationNameLength + 2);
        var payloadBytes = this.payloadMessage.payloadBytes;
        remLength = remLength + payloadBytes.byteLength;
      ...
      pos = writeString(this.payloadMessage.destinationName, destinationNameLength, byteStream, pos);
        ...
        switch (this.mqttVersion) {
          case 3:
            ...
            pos = pos + MqttProtoIdentifierv3.length;
            break;

          case 4:
            byteStream.set(MqttProtoIdentifierv4, pos);
            pos = pos + MqttProtoIdentifierv4.length;
            break;
        }
        ...
        if (this.willMessage != undefined) {
          connectFlags = connectFlags | 0x04;
          connectFlags = connectFlags | this.willMessage.qos << 3;

          if (this.willMessage.retained) {
            connectFlags = connectFlags | 0x20;
          }
        }
    ...
    switch (this.type) {
      case MESSAGE_TYPE.CONNECT:
        ...
        if (this.willMessage != undefined) {
          ...
          pos = pos + willMessagePayloadBytes.byteLength;
        }
  ...
  function decodeMessage(input, pos) {
    ...
    var first = input[pos];
    var type = first >> 4;
    var messageInfo = first = first & 0x0f;
    pos = pos + 1; // Decode the remaining length (MBI format)
    ...
    do {
      ...
      digit = input[pos++];
      remLength = remLength + (digit & 0x7F) * multiplier;
    ...
    switch (type) {
      ...
      case MESSAGE_TYPE.PUBLISH:
        ...
        pos = pos + len; // If QoS 1 or 2 there will be a messageIdentifier
  ...
  function parseUTF8(input, offset, length) {
    ...
    while (pos < offset + length) {
      ...
      if (utf16 > 0xFFFF) // 4 byte character - express as a surrogate pair
        {
          utf16 = utf16 - 0x10000;
          output = output + String.fromCharCode(0xD800 + (utf16 >> 10)); // lead character

          utf16 = 0xDC00 + (utf16 & 0x3FF); // trail character
        }

      output = output + String.fromCharCode(utf16);
    }
  ...
  */
  ...
  var ClientImpl = function ClientImpl(uri, host, port, path, clientId) {
    ...
    for (var _key in localStorage) {
      if (_key.indexOf('Sent:' + this._localKey) == 0 || _key.indexOf('Received:' + this._localKey) == 0) {
        this.restore(_key);
      }
    }
  ...
  ClientImpl.prototype._deframeMessages = function (data) {
    ...
    try {
      ...
      while (offset < byteArray.length) {
        ...
        var _wireMessage = result[0];
        offset = result[1];

        if (_wireMessage !== null) {
          messages.push(_wireMessage);
        } else {
          break;
        }
      }
  ...
  */
  ...
  var Client = function Client(host, port, path, clientId) {
    ...
    if (typeof host !== 'string') {
      throw new Error(format(ERROR.INVALID_TYPE, [_typeof(host), 'host']));
    }
    ...
    if (arguments.length == 2) {
      ...
      if (typeof port !== 'number' || port < 0) {
        throw new Error(format(ERROR.INVALID_TYPE, [_typeof(port), 'port']));
      }

      if (typeof path !== 'string') {
        throw new Error(format(ERROR.INVALID_TYPE, [_typeof(path), 'path']));
      }
    ...
    this._setOnConnectionLost = function (newOnConnectionLost) {
      if (typeof newOnConnectionLost === 'function') {
        ...
        throw new Error(format(ERROR.INVALID_TYPE, [_typeof(newOnConnectionLost), 'onConnectionLost']));
      }
    };
    ...
    this._setOnMessageDelivered = function (newOnMessageDelivered) {
      if (typeof newOnMessageDelivered === 'function') {
        ...
        throw new Error(format(ERROR.INVALID_TYPE, [_typeof(newOnMessageDelivered), 'onMessageDelivered']));
      }
    };
    ...
    this._setOnMessageArrived = function (newOnMessageArrived) {
      if (typeof newOnMessageArrived === 'function') {
        ...
        throw new Error(format(ERROR.INVALID_TYPE, [_typeof(newOnMessageArrived), 'onMessageArrived']));
      }
    };
    ...
    this.connect = function (connectOptions) {
      ...
      if (connectOptions.willMessage) {
        ...
        if (typeof connectOptions.willMessage.destinationName === 'undefined') {
          throw new Error(format(ERROR.INVALID_TYPE, [_typeof(connectOptions.willMessage.destinationName), 'connectOptions.willMessage.destinationName']));
        }
      }
      ...
      if (connectOptions.hosts) {
        ...
        for (var i = 0; i < connectOptions.hosts.length; i++) {
          if (typeof connectOptions.hosts[i] !== 'string') {
            throw new Error(format(ERROR.INVALID_TYPE, [_typeof(connectOptions.hosts[i]), 'connectOptions.hosts[' + i + ']']));
          }
        ...
        if (!usingURIs) {
          ...
          for (var i = 0; i < connectOptions.hosts.length; i++) {
            if (typeof connectOptions.ports[i] !== 'number' || connectOptions.ports[i] < 0) {
              throw new Error(format(ERROR.INVALID_TYPE, [_typeof(connectOptions.ports[i]), 'connectOptions.ports[' + i + ']']));
            }

            var _host = connectOptions.hosts[i];
            var _port = connectOptions.ports[i];
            var ipv6 = _host.indexOf(':') != -1;
            uri = 'ws://' + (ipv6 ? '[' + _host + ']' : _host) + ':' + _port + path;
    ...
    this.send = function (topic, payload, qos, retained) {
      ...
      if (arguments.length == 0) {
        ...
        if (!(topic instanceof Message) && typeof topic !== 'string') {
          throw new Error('Invalid argument:' + _typeof(topic));
        }
