...
//   const Paho = {}; // eslint-disable-line no-unused-vars
// }
const Paho = {};
...
Paho.MQTT = (function (global) {
  ...
  /**
	 ...
	 * @private
	 * @param {error} error ERROR.KEY value above.
	 * @param {array} substitutions [array] substituted into the text.
	 * @returns {string} the text with the substitutions made.
	 */
  ...
  /**
	 ...
	 * @returns {void}
	 */
  let validate = function (obj, keys) {
    for (let key in obj) {
  ...
  /**
	 ...
	 * to a fixed scope.
	 * @param {function} f User function
	 * @param {object} scope Function scope
	 * @return {function} User function bound to another scope
	 * @private
	 */
  let scope = function (f, scope) {
    return function () {
  ...
  let MqttProtoIdentifierv3 = [0x00, 0x06, 0x4d, 0x51, 0x49, 0x73, 0x64, 0x70, 0x03];
  //MQTT proto/version for 311         4    M    Q    T    T    4
  let MqttProtoIdentifierv4 = [0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04];

  /**
	 * Construct an MQTT wire protocol message.
	 * @param {string} type MQTT packet type.
	 * @param {object} options optional wire message attributes.
	 * @returns {void}
	 *
	 * Optional properties
	 *
	 ...
	 * @ignore
	 */
  let WireMessage = function (type, options) {
    this.type = type;
  ...
  WireMessage.prototype.encode = function () {
    ...
    if (typeof this.messageIdentifier !== 'undefined') {
    ...
    case MESSAGE_TYPE.CONNECT:
      ...
      if (this.willMessage !== undefined) {
        remLength = remLength + (UTF8Length(this.willMessage.destinationName) + 2);
  ...
  function decodeMessage(input, pos) {
    ...
    let first = input[pos];
    let type = first >> 4;
    let messageInfo = first = first & 0x0f;
    pos = pos + 1;
  ...
  /**
	 ...
	 * @ignore
	 */
  let Pinger = function (client, window, keepAliveInterval) {
    this._client = client;
    this._window = window;
  ...
  /**
	 ...
	 * @ignore
	 */
  let Timeout = function (client, window, timeoutSeconds, action, args) {
    this._window = window;
    if (!timeoutSeconds) {
      timeoutSeconds = 30;
    }

    let doTimeout = function (action, client, args) {
      return function () {
  ...
  /*
	 ...
	 */
  let ClientImpl = function (uri, host, port, path, clientId) {
  ...
  ClientImpl.prototype.store = function (prefix, wireMessage) {
    let storedMessage = {type: wireMessage.type, messageIdentifier: wireMessage.messageIdentifier, version: 1};
  ...
  ClientImpl.prototype._process_queue = function () {
    ...
    let fifo = this._msg_queue.reverse();
  ...
  ClientImpl.prototype._on_socket_message = function (event) {
    ...
    let messages = this._deframeMessages(event.data);
  ...
  ClientImpl.prototype._trace = function () {
    ...
    if (this.traceFunction) {
      ...
      let record = Array.prototype.slice.call(arguments).join('');
  ...
  /**
	 ...
	 *                            </ol>
	 */
  let Client = function (host, port, path, clientId) {
	    let uri;
    ...
    let client = new ClientImpl(uri, host, port, path, clientId);
